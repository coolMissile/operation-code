...
这是一个简化的Christofides算法，包含以下步骤：
1. 使用Kruskal算法构建最小生成树
2. 贪心连接生成树
3. 使用2-opt进行局部优化(只有3轮？)

注意：这不是完整的Christofides算法，缺少：
1. 奇数度顶点的完美匹配
2. 欧拉回路的正确构造
...
if __name__ == "__main__":
    ...
    import random
    N = 10
    distances = [ [random.random() for j in range(i)] for i in range(N) ]
    path = solve_tsp( distances, optim_steps=10 )
    print("Path:", path)